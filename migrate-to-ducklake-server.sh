...
# -----------------------------------------------------------------------------
# Script: migrate-to-ducklake-server.sh
# Description: Migrates existing DuckDB database to DuckLake format
#              FOR SERVER (grafana.safecast.jp)
# -----------------------------------------------------------------------------

set -e

echo "=========================================="
echo "Migrating to DuckLake (SERVER)"
echo "=========================================="
echo ""
...
# Find DuckDB binary (server locations)
if [ -x "/root/.local/bin/duckdb" ]; then
    ...
    echo "   Checked: /root/.local/bin/duckdb, system PATH, $HOME/.local/bin/duckdb"
    exit 1
fi

echo "✅ Found DuckDB at: $DUCKDB_BIN"
echo "📍 Environment: SERVER ($(hostname))"
echo ""

# Safety check - confirm this is the server
if [[ $(hostname) != *"safecast"* ]] && [[ $(hostname) != "vps-01" ]]; then
    echo "⚠️  WARNING: This doesn't appear to be the Safecast server!"
...
if [ -f "$OLD_DB" ]; then
    cp "$OLD_DB" "${OLD_DB}.backup_$(date +%Y%m%d_%H%M%S)"
...
chown -R grafana:grafana "$DUCKLAKE_DATA"
chmod 775 "$DUCKLAKE_DATA"
...
chown grafana:grafana "${DUCKLAKE_CATALOG}"*
chmod 664 "${DUCKLAKE_CATALOG}"*
...
chown -R grafana:grafana "${DUCKLAKE_CATALOG}"*
chown -R grafana:grafana "$DUCKLAKE_DATA"
chmod 664 "${DUCKLAKE_CATALOG}"*
chmod 775 "$DUCKLAKE_DATA"
...
echo ""
echo "2. Update Grafana datasource (https://grafana.safecast.jp):"
...
echo "   - Open Grafana dashboard in browser"
echo "   - Run: ./devices-last-24-hours-ducklake.sh"
echo "   - Both should work simultaneously!"
echo ""
echo "5. Update cron job:"
echo "   crontab -e"
echo "   Change: ./devices-last-24-hours.sh"
echo "   To:     ./devices-last-24-hours-ducklake.sh"
echo ""
